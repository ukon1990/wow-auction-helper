...
export const updateTSMData = middyfy(async ({apiKey}: {apiKey: string}): Promise<ValidatedEventAPIGatewayProxyEvent<any>> => {
  let response;
  try {
    await new TsmService(apiKey).updateAllRegions();
